...
@section('content')
    <div class="content-wrapper">
        ...
        {!! Form::open(['route' => 'salary.approved.apply','method' => 'POST']) !!}
        <div class="row">
                            ...
                            <table class="table table-bordered">
                                ...
                                @foreach($salaries as $salary)
                                    <tr>
                                        ...
                                        <td>{{$salary->pdf}}</td>
                                        <td>
                                            <a href="{{asset('assets/images/salary_document/'.$salary->pdf)}}" class="btn btn-inverse-success btn-icon" target="_blank"><br/><i class="mdi mdi-eye"></i></a>
                                        </td>
                                        <td>
                                            @if($salary->status == 0)
                                                {!! Form::open(['route' => 'approved.salary','method' => 'POST']) !!}
                                                <input type="hidden" value="{{$salary->id}}" name="id"/>
                                                <button type="submit" class="btn btn-danger btn-fw">Pending</button>
                                                {!! Form::close() !!}
                                            @else
