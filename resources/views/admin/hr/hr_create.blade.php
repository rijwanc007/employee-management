...
@section('content')
    <div class="content-wrapper">
        {!! Form::open(['class' =>'form-sample','route' => 'hr.store','method' => 'POST','enctype' => 'multipart/form-data']) !!}
                            ...
                            <div class="col-sm-10">
                                <select class="form-control" name="role" required>
                                    @foreach($roles as $role)
                                        <option value="{{$role->id}}" @if( $role->role_name == 'hr') selected @endif>{{$role->role_name}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
