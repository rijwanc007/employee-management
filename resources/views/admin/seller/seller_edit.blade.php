...
@section('content')
    <div class="content-wrapper">
        {!! Form::open(['class' =>'form-sample','route' => ['seller.update',$edit->id],'method' => 'PATCH','enctype' => 'multipart/form-data']) !!}
        <div class="page-header" id="bannerClose"><h3 class="page-title"><span class="page-title-icon bg-gradient-primary text-white mr-2"><i class="mdi mdi-account"></i></span> Update Seller</h3></div>
        <div class="row">
            <div class="col-md-6">
                <div class="card">
                    <div class="card-body">
                        <input type="hidden" value="seller" name="account_for"/>
                        ...
                        <div class="form-group private_email">
                            ...
                            <input type="email" class="form-control" id="private_email" name="private_email" value="{{$edit->private_email}}" placeholder="Private Email" >
                        </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="employee_type" name="employee_type" required>
                                ...
                                <option value="full_time" @if($edit->employee_type == 'full_time') selected @endif>Full Time</option>
                                <option value="half_time" @if($edit->employee_type == 'half_time') selected @endif>Half Time</option>
                                <option value="per_hour" @if($edit->employee_type == 'per_hour') selected @endif>Per Hour</option>
                                <option value="per_project" @if($edit->employee_type == 'per_project') selected @endif>Per Project</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <?php
                            $get_supervisor = explode("_", $edit->work_under);
                            ?>
                            <select class="form-control" id="work_under" name="work_under" required>
                                <option selected disabled value="">Choose An Option</option>
                                @foreach($supervisors as $supervisor)
                                    <option value="{{$supervisor->name}}_{{$supervisor->id}}" @if( $supervisor->id == $get_supervisor[1]) selected @endif>{{$supervisor->name}}({{$supervisor->email}})</option>
                                @endforeach
                            </select>
                        </div>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <button type="submit" class="btn btn-gradient-primary btn-lg btn-block"><i class="mdi mdi-account"></i> Update Seller </button>
            </div>
        </div>
        {!! Form::close() !!}
    </div>
